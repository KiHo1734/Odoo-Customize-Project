...
def test_camera(model_dir, device_id, cam_id=0):
    model_test = AntiSpoofPredict(device_id)
    image_cropper = CropImage()
    ...
    while True:
        ...
        h, w, _ = frame.shape
        if w / h != 3 / 4:
            frame = cv2.resize(frame, (int(h * 3 / 4), h))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Anti Spoof Test")
    ...
    if args.camera:
        test_camera(args.model_dir, args.device_id)
    else:
